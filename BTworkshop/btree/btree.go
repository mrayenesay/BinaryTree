...
func (n *Node) ForceToTree(rate int) int {
	...
	if rnd.Intn(100) > (100 - rate) {
		if number := rand.Intn(n.Value + 1); number != n.Value {
			total++
			n.Value = number
		}
	}
...
func (n *Node) BinaryTreeJson() string {
	if n.Right == nil || n.Left == nil {
		return fmt.Sprintf("{\"value\":%d}", n.Value)
	}
	return fmt.Sprintf("{\"value\":%d}, \"left\":%s, \"right\":%s}", n.Value, n.Left.BinaryTreeJson(), n.Left.BinaryTreeJson())
}
func (n *Node) SaveTree(filename string) error {
	result := n.BinaryTreeJson()
